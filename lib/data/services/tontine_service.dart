...
class TontineService {
  ...
  Future<bool> makeContribution(
    ...
    try {
      ...
      return await _firestore.runTransaction((transaction) async {
        ...
        final member = tontine.members.firstWhere((m) => m.userId == userId);
        ...
        final updatedCycles = tontine.cycles.map((cycle) {
          if (cycle.cycleNumber == cycleNumber) {
            // Créer un nouveau cycle avec la contribution ajoutée
            return TontineCycle(
  ...
  List<TontineCycle> _generateCycles(
    ...
    for (int i = 0; i < tontine.currentMembers; i++) {
      ...
      final winnerId = memberOrder[i];
      final winner = tontine.members.firstWhere((m) => m.userId == winnerId);
      ...
      cycles.add(
        TontineCycle(
          ...
          beneficiaryName: winner.name,
          startDate: cycleDate,
          endDate: cycleDate.add(Duration(days: frequencyDays)),
          payoutDate: cycleDate.add(Duration(days: frequencyDays - 1)),
